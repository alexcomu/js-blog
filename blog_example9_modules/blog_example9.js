...
var myModule = require('./blog_modules');
...
dispatcher.onPost("/", function(req, res) {
  ...
  blog.sort(myModule.mySortFunction);
...
dispatcher.onPost("/delete", function(req, res) {
  if(req.params.blogid){
    ...
    var index = blog.indexOf(myModule.getBlogById(blog,req.params.blogid));
...
dispatcher.onGet("/blog", function(req, res){
  if(req.params.id){
    var myBlog = myModule.getBlogById(blog, req.params.id);
    if(myBlog){
      res.writeHead(200, {'Content-Type': 'text/html'});
      res.write('<html><head><title>Welcome to My Blog</title></head>');
